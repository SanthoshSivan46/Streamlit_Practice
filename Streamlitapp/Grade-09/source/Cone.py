...
def cone_1():
    head.title()
    st.markdown("<p style='text-align: center; color: black; font-size:20px;'><span style='font-weight: bold'>Problem Statement: </span>Application to find the Surface Area and Volume of the Cone </p>", unsafe_allow_html=True)
    ...
    with bc2:
        ...
        st.markdown("")
        def clear_text():
            st.session_state["Clear_cone_Radius"] = 0
            st.session_state["Clear_cone_Height"] = 0
            st.session_state["text1_selectbox"] = "Surface Area"
            ...
            with col2:
                if vAR_input_Radius and vAR_input_Height  != 0:
                    if selected == "Surface Area":
                        vAR_area= ( 3.14 * vAR_input_Radius * (vAR_input_Height + vAR_input_Radius) )     
                        vAR_area = round (vAR_area,2)
                        st.success(vAR_area)
                    if selected == "Volume":
                        vAR_area= ( (1/3) * 3.14 *  vAR_input_Radius**2 * vAR_input_Height )     
                        vAR_area = round (vAR_area,2)
                        st.success(vAR_area)
                else:
                    st.error("Error")
